...
(* donated
     : nat_eqType -> value -> cstate crowdState -> bool *)
 ...
 ************************************************************************)
(* Lemma 2 *)
(* Ilya's claim: doesn't capture whether the contract records the amount
that the backer has provided *)
(* Therefore, I can change the amount recorded in the donate function
and this shouldn't break *)
(* In any case, the binary q is unnecessary since it's just a hole *)
...
(*   elim=>[|[bc m] sc Hi] st st' P R.
  (* This performs induction on the scheduler *)
  (* In the base case, the step protocol does nothing and returns st *)
  - unfold reachable'.
    simpl.
    intro steq.
    subst st'.
    exact P.
  - unfold reachable'. intro.
    apply (Hi (post (step_prot crowd_prot st bc m))).
    Focus 2.
intro b0. intro Hb0.
apply (R b0).
apply In_cons. right; exact Hb0.
unfold donated.
unfold donated in P.
(* How does the argument proceed from here? *)
simpl in H. *)
...
TODO: if enough people donate, owner can kickstart campaign
...
(* Lemma 4 *)
(* If enough people donate, the owner can kickstart the campaign *)
Definition goal_met (st : cstate crowdState) : Prop :=
  sumn (map snd (backers (state st))) >= get_goal (state st).

(* This is slightly different from the can_claim_back lemma in that 
can_claim_back assumes the owner has already swept the donation array
and transferred all the funds to contract balance *)
Lemma owner_gets_funds st bc :
  get_max_block (state st) < block_num bc ->
  goal_met st ->
  ~~(funded (state st)) ->
  exists (m : message),
    sender m == get_owner (state st) /\
    out (step_prot crowd_prot st bc m) =
    Some (Msg (balance st) crowd_addr (get_owner (state st)) 0 ok_msg).
Proof.
move=>Nb Nm Nf.
exists (Msg 0 (get_owner (state st)) crowd_addr getfunds_tag [::]); split=>//.
rewrite /step_prot.
case: st Nb Nm Nf=>id bal s/= Nb Nm Nf.
rewrite /apply_prot/=/getfunds_fun/=.
(* How do I get rid of the get_owner s == get_owner s that is
clearly true? ssreflect helpline? *)
replace (get_owner s == get_owner s) with true.
replace (get_max_block s < block_num bc + 1) with true.
unfold goal_met in Nm.
simpl in Nm. 
SearchAbout sumn. (* How do I link the balance to the array contents? *)
(* Stuck... *)
(* I need to prove that get_goal s <= bal *)
(* Based on the fact that Nm *)
(* Because I have used all the other hypotheses *)
Focus 2.
admit.
Admitted.
*)
